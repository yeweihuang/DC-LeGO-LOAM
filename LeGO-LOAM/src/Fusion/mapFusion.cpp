...
//    lio_sam::context_info _context_info;
    ...
    int matched_cnt, loop_info_cnt, trans_odom_cnt;
...
public:
    ...
    MapFusion(){
        ...
        _sub_laser_cloud_info = nh.subscribe<lio_sam::cloud_info>("/" + _robot_id + "/" + _local_topic,
                100, &MapFusion::laserCloudInfoHandler, this, ros::TransportHints().tcpNoDelay());
        ...
        if(_robot_id != _robot_initial){
            _sub_scan_context_info = nh.subscribe<lio_sam::context_info>("/" + _sc_topic + "/context_info",
                 10000, &MapFusion::scanContextInfoHandler, this, ros::TransportHints().tcpNoDelay());//number of buffer may differs for different robot numbers
            _sub_odom_trans = nh.subscribe<nav_msgs::Odometry>(_sc_topic + "/trans_odom", 200, &MapFusion::OdomTransHandler, this, ros::TransportHints().tcpNoDelay());
		...
		_pub_trans_odom2map   = nh.advertise<nav_msgs::Odometry> ("/" + _robot_id + "/" + _sc_topic + "/trans", 1);
...
private:
    void ParamLoader(){
        ...
        nh.getParam("/mapfusion/interRobot/pcm_matrix_folder",_pcm_matrix_folder);
        ...
        nh.getParam("/mapfusion/interRobot/pcm_start_threshold", _pcm_start_thres );
    ...
    void initialization(){
		...
		_downsize_filter_icp.setLeafSize(0.8, 0.8, 0.8);

        _initial_loop.first = -1;

//        _context_info.robotID = _robot_id;
//
//        _context_info.numRing = _knn_feature_dim;
//        _context_info.numSector = _num_sectors;
//
//        _context_info.scanContextBin.assign(_knn_feature_dim * _num_sectors,0);
//        _context_info.ringKey.assign(_knn_feature_dim, 0);
...
//        _signal_2 = false;
        ...
        _trans_to_publish.x = 10;
        _trans_to_publish.y = 0;
        _trans_to_publish.z = 0;
        _trans_to_publish.roll  = 0;
        _trans_to_publish.pitch = 0;
        _trans_to_publish.yaw   = 0;
    }
    ...
    void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr& msgIn)
    {
        ...
        pcl::fromROSMsg(msgIn->cloud_corner, *_laser_cloud_feature);
...
//        recordProcessingTime(time0, ros::Time::now().toSec(), "SC");
...
//        std::cout<< _robot_id <<" received "<< _global_odom_trans[index].size()<<std::endl;
...
//                std::cout<<"id_t"<<id_t<<std::endl;
        ...
        gtsam::writeG2o( graph,result,"/home/yewei/catkin_ws/data/our/" + _robot_id + "_factor_graph.g2o" );
        ...
        if (_trans_to_publish.x != 0 || _trans_to_publish.y != 0 || _trans_to_publish.z != 0){
            ...
            tf::Transform map_to_odom = tf::Transform(tf::createQuaternionFromRPY(_trans_to_publish.roll, _trans_to_publish.pitch, _trans_to_publish.yaw), tf::Vector3(_trans_to_publish.x, _trans_to_publish.y, _trans_to_publish.z));
            tfMap2Odom.sendTransform(tf::StampedTransform(map_to_odom, ros::Time::now(), "/map", "/" + _robot_id + "/odom"));
        }
...
//        std::cout<<"with factor graph: "<< _trans_to_publish.x <<" ," << _trans_to_publish.y <<" ," <<_trans_to_publish.z <<" ," <<
//            _trans_to_publish.roll <<" ," << _trans_to_publish.pitch <<" ," << _trans_to_publish.yaw <<std::endl;
...
//        result.print();
    ...
    void recordProcessingTime(double start, double end, string label){
        double seconds = end - start;
        //writing to the time to file
        _processing_time_list.push_back(std::make_pair(label, seconds));
        std::ofstream outfile;
        std::string outname = "/home/yewei/catkin_ws/" + _robot_id + "processingtime.csv";
        outfile.open (outname);
        for (auto& ite : _processing_time_list)
            outfile << ite.first << "," << ite.second << std::endl;
    ...
    void run(ScanContextBin bin){
        ...
        auto time0 = ros::Time::now().toSec();
        buildKDTree(bin);
        KNNSearch(bin);
//        recordProcessingTime(time0, ros::Time::now().toSec(), "KDtree00");
        auto time01 = ros::Time::now().toSec();

//        if(_idx_nearest_list.empty())
//            distanceSearch(bin);

//        recordProcessingTime(time01, ros::Time::now().toSec(), "KDtree01");
        auto time02 = ros::Time::now().toSec();

        if(!getInitialGuesses(bin)) {
//            recordProcessingTime(time02, ros::Time::now().toSec(), "KDtree02");
//            recordProcessingTime(time0, ros::Time::now().toSec(), "failure");
            return;
        }
        else{
            matched_cnt++;
            std::cout<<_robot_id<<" matched_cnt :"<<matched_cnt<<std::endl;
        }
...
//        recordProcessingTime(time02, ros::Time::now().toSec(), "KDtree02");
        ...
        if(!incrementalPCM()){
//            recordProcessingTime(time0, ros::Time::now().toSec(), "failure");
//            recordProcessingTime(time1, ros::Time::now().toSec(), "PCM");
            return;
        }
...
//        recordProcessingTime(time1, ros::Time::now().toSec(), "PCM");
        ...
        printloopinfo();
        auto time3 = ros::Time::now().toSec();


        //perform optimization
        gtsamExpressionGraph();

//        recordProcessingTime(time3, ros::Time::now().toSec(), "Global");
//        recordProcessingTime(time0, ros::Time::now().toSec(), "success");
...
//        std::cout<< "robot received: " << bin_nearest.robotname <<std::endl;

        //get initial guess from scancontext
        target_pose = bin_nearest.pose;

//        recordProcessingTime(time0, ros::Time::now().toSec(), "initial0");
...
//        recordProcessingTime(time0, ros::Time::now().toSec(), "initial1");
        time0 = ros::Time::now().toSec();
        ...
        std::cout<< bin.robotname << bin.pose.intensity <<" matched with :" << bin_nearest.robotname << bin_nearest.pose.intensity <<std::endl;
//        recordProcessingTime(time0, ros::Time::now().toSec(), "initial2");
        time0 = ros::Time::now().toSec();
...
//        recordProcessingTime(time0, ros::Time::now().toSec(), "initial3");
        time0 = ros::Time::now().toSec();
...
//        recordProcessingTime(time0, ros::Time::now().toSec(), "icp0");
        time0 = ros::Time::now().toSec();
...
//        recordProcessingTime(time0, ros::Time::now().toSec(), "icp1");
        time0 = ros::Time::now().toSec();
...
//        recordProcessingTime(time0, ros::Time::now().toSec(), "icp2");
    ...
    bool incrementalPCM() {
        if (_pose_queue[_robot_this_th].size() < _pcm_start_thres)
            return false;
//        std::cout<<"incremental between "<< _robot_id << " & " << _robot_this <<std::endl;
    ...
    void gtsamExpressionGraph(){
        ...
        //result.print("Final Optimize Result:\n");
...
//        std::cout<<"with gtsam: "<< map_trans_this.x <<" ," << map_trans_this.y <<" ," <<map_trans_this.z <<" ," <<
//            map_trans_this.roll <<" ," << map_trans_this.pitch <<" ," << map_trans_this.yaw <<std::endl;
    }
...
//        std::cout<<_robot_id<<" sending map messages..."<<std::endl;
//        std::cout<< _trans_to_publish.x <<" ," << _trans_to_publish.y <<" ," <<_trans_to_publish.z <<" ," <<
//                  _trans_to_publish.roll <<" ," << _trans_to_publish.pitch <<" ," << _trans_to_publish.yaw <<std::endl;
        ...
        odom2map.child_frame_id = _robot_id + "/odom";
    ...
    void sendLoopThis(int robot_id_this, int robot_id_that, int id_loop_this, int id_loop_that){
        ...
        if(robot_id_this == robot_id_that){
            ...
            cout<<trans_this3.translation()<<endl;
            cout<<trans_that3.translation()<<endl;
            cout<<pose_to_this.translation()<<endl;
            cout<<pose_to_that.translation()<<endl;
        }
        ...
        cout << "calculated: "<< dpose.translation()<<endl;


        cout<<"real: "<< dpose1.translation() << endl;

    }

    void sendLoopThat(int robot_id_this,int robot_id_that, int id_loop_this, int id_loop_that){
        loop_info_cnt ++;
        std::cout<<_robot_id<<"loop_info: "<< loop_info_cnt<<std::endl;
        if(robot_id_this != robot_id_that)
            return;
    ...
    void sendOdomOutputMessage(){
        ...
        trans_odom_cnt++;
        std::cout<<_robot_id<<"trans_odom: "<< trans_odom_cnt <<std::endl;


        sendGlobalLoopMessage();

    }

    void printloopinfo(){
        // Write to file
        std::string file_name = _pcm_matrix_folder + "/" + _robot_id + "_loop.txt";
        std::ofstream output_file;
        output_file.open(file_name);

        int id_0, id_1;

        for (auto list_this : _loop_accept_queue){
            id_0 = std::min(list_this.first, _robot_id_th);
            id_1 = std::max(list_this.first, _robot_id_th);
            auto loop_queue_this = _loop_queue[list_this.first];
            for (auto id : list_this.second){
                auto loop_record_this = loop_queue_this[id];
                gtsam::Pose3 pose_this = std::get<2>(loop_record_this);
                output_file << id_0 <<" "<< id_1<<" "<<
                    _bin_with_id[std::get<0>(loop_record_this)].pose.intensity <<" "<<
                        _bin_with_id[std::get<1>(loop_record_this)].pose.intensity <<" "<<
                            pose_this.translation().x() <<" "<<pose_this.translation().y()<<" "<<pose_this.translation().z()<<" "<<
                                pose_this.rotation().roll() <<" "<<pose_this.rotation().pitch()<<" "<<pose_this.rotation().yaw()<<" "<<1<<std::endl;
            }
        }
        output_file.close();

        gtsam::NonlinearFactorGraph door_loop;//
//        for (auto list_this: _loop_queue){
//            for (int i = 0; i < list_this.second.size(); i++){
//                auto tuple_this = list_this.second[i];
//                auto bin0 = _bin_with_id[std::get<0>(tuple_this)];
//                auto bin1 = _bin_with_id[std::get<1>(tuple_this)];
//                char c_0 = 'a' + bin0.robotname.back() -'0';
//                char c_1 = 'a' + bin1.robotname.back() -'0';
//                gtsam::Symbol s_0(c_0, bin0.pose.intensity);
//                gtsam::Symbol s_1(c_1, bin1.pose.intensity);
//                gtsam::Pose3 dpose = std::get<2>(tuple_this);
//
//                gtsam::Vector Vector6(6);
//                float noiseScore = std::get<2>(_pose_queue[list_this.first][i]);
//                Vector6 << noiseScore, noiseScore, noiseScore, noiseScore, noiseScore, noiseScore;
//                gtsam::noiseModel::Diagonal::shared_ptr constraintNoise = gtsam::noiseModel::Diagonal::Variances(Vector6);
//                door_loop.add(gtsam::BetweenFactor<gtsam::Pose3>(s_0, s_1, dpose, constraintNoise));
//            }
//        }
        for (auto list_this: _loop_accept_queue){
            auto loop_queue_this = _loop_queue[list_this.first];
            for (auto id : list_this.second){
                auto loop_record_this = loop_queue_this[id];
                auto bin0 = _bin_with_id[std::get<0>(loop_record_this)];
                auto bin1 = _bin_with_id[std::get<1>(loop_record_this)];
                char c_0 = 'a' + bin0.robotname.back() -'0';
                char c_1 = 'a' + bin1.robotname.back() -'0';
                gtsam::Symbol s_0(c_0, bin0.pose.intensity);
                gtsam::Symbol s_1(c_1, bin1.pose.intensity);
                gtsam::Pose3 dpose = std::get<2>(loop_record_this);

                gtsam::Vector Vector6(6);
                float noiseScore = std::get<2>(_pose_queue[list_this.first][id]);
                Vector6 << noiseScore, noiseScore, noiseScore, noiseScore, noiseScore, noiseScore;
                gtsam::noiseModel::Diagonal::shared_ptr constraintNoise = gtsam::noiseModel::Diagonal::Variances(Vector6);
                door_loop.add(gtsam::BetweenFactor<gtsam::Pose3>(s_0, s_1, dpose, constraintNoise));
            }
        ...
        gtsam::Values v;
        gtsam::writeG2o(door_loop,v, "/home/yewei/catkin_ws/data/door/tmp/" + string(1, _robot_id.back()) + ".g2o");
